class VoronoiDebug {
	constructor(svg_context, debug = false) {
		...
		Parabola.svg_context = this._svg_;
...
*/
  ...
  */
	...
	partial_update(d){
		...
		if (this.debug) {
			this.directrix.attr("y1", d).attr("y2", d);

			// Check next event
			if (
				this.event_list._list.length > 0 &&
				this.event_list._list[0].position <= d
			) {
				let e = this.event_list.removeFirst();
	...
	addDirectrix() {
        ...
        let obj = this; 
		let dd = $(document.createElementNS("http://www.w3.org/2000/svg", "line"))
			.attr({
				y1: 0,
				y2: 0,
				x1: 0,
